...
export class LoginComponent {
    ...
    userdata:any;
  constructor( private Authentication:AuthencationService,
  ...
  proceedlogin(){
    ...
    if(this.loginForm.valid){
        this.Authentication.login(this.loginForm.value).subscribe((res:any)=>{
            console.log("ressssssss", res);
            this.userdata = res?.data;
            console.log('data userrrrrr',this.userdata);
               if(this.userdata){
                    this.HelperSeriveService.setItem('accessToken', this.userdata.accessToken);
                    this.HelperSeriveService.setItem('user', this.userdata.user);
                    window.location.href= '/'
               }else{
                    this.ToastrService.error("Please contract admin","In active user");
                }
